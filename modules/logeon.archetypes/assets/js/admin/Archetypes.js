(function () {
    ...
    var AdminArchetypes = {
        ...
        bindModalShown: function () {
            var self = this;
            this.modalNode.addEventListener('shown.bs.modal', function () {
                self.refreshRichtextFields();
            });
        },

        refreshRichtextFields: function () {
            if (!this.modalForm || typeof window.$ !== 'function') { return; }
            var self = this;
            ['description', 'lore_text'].forEach(function (name) {
                var el = self.modalForm.querySelector('[name="' + name + '"]');
                if (el) {
                    try { window.$(el).summernote('code', el.value); } catch (e) {}
                }
            });
        },
