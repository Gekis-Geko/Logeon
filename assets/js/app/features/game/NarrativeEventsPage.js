    ...
    var page = {
        ...
        closeScene: function (eventId, btn) {
            ...
            mod.close({ event_id: eventId }).then(function () {
                if (globalWindow.Toast && typeof globalWindow.Toast.show === 'function') {
                    globalWindow.Toast.show({ body: 'Scena chiusa.', type: 'success' });
                }
                ...
                if (globalWindow.Toast && typeof globalWindow.Toast.show === 'function') {
                    globalWindow.Toast.show({ body: msg, type: 'error' });
                }
            });
        },
        ...
        render: function (rows) {
            ...
            list.innerHTML = rows.map(function (ev) {
                ...
                var label = sourceLabels[source] || self.humanizeSourceLabel(source);
        ...
        humanizeSourceLabel: function (source) {
            var normalized = String(source || '').trim().toLowerCase();
            if (!normalized) {
                return 'Evento';
            }
            var words = normalized.replace(/[_-]+/g, ' ').split(/\s+/);
        ...
        showCreateModal: function () {
            ...
            var modal = globalWindow.bootstrap && globalWindow.bootstrap.Modal
                ? globalWindow.bootstrap.Modal.getOrCreateInstance(modalEl)
                : null;
            if (modal) { modal.show(); }
        },
        ...
        submitCreate: function () {
            ...
            mod.create({ title: title, description: desc, location_id: locationId, scope: scope, impact_level: impact })
                .then(function (response) {
                    ...
                    var modalEl = document.getElementById('modalCreateNarrativeEvent');
                    var modal = globalWindow.bootstrap && globalWindow.bootstrap.Modal
                        ? globalWindow.bootstrap.Modal.getInstance(modalEl)
                        : null;
                    if (modal) { modal.hide(); }

                    var msg = (response && response.message) ? response.message : 'Evento creato con successo.';
                    if (globalWindow.Toast && typeof globalWindow.Toast.show === 'function') {
                        globalWindow.Toast.show({ body: msg, type: 'success' });
                    }
...
globalWindow.GameNarrativeEventsPage = GameNarrativeEventsPage;
export { GameNarrativeEventsPage as GameNarrativeEventsPage };
export default GameNarrativeEventsPage;
