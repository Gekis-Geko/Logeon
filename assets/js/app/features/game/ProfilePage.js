(function (window) {
    ...
    function normalizeProfileError(error, fallback) {
        ...
        if (window.GameFeatureError && typeof window.GameFeatureError.info === 'function') {
            var info = window.GameFeatureError.info(error, fb) || {};
        ...
        if (window.GameFeatureError && typeof window.GameFeatureError.normalize === 'function') {
            return window.GameFeatureError.normalize(error, fb);
        }
    ...
    function GameProfilePage(character_id, extension) {
            let page = {
                ...
                renderGuilds: function (block, characterId) {
                    ...
                    var http = (window.Request && window.Request.http) ? window.Request.http : null;
                ...
                renderArchetypes: function (block) {
                    var archetypeItem = block.find('[data-archetype-item]');
                    if (!archetypeItem.length) { return; }
                    ...
                    http.post('/archetypes/my', {}).then(function (response) {
                        ...
                        var label = list.length > 1 ? 'Archetipi' : 'Archetipo';
                        archetypeItem.find('[name="archetype_label"]').text(label);

                        var iconsContainer = archetypeItem.find('[name="archetype_icons"]');
                        iconsContainer.empty();
                        var hasContent = false;
                        list.forEach(function (a) {
                            if (!a) { return; }
                            var name = (a.name ? String(a.name).trim() : '');
                            var icon = (a.icon ? String(a.icon).trim() : '');
                            if (icon !== '') {
                                hasContent = true;
                                var img = $('<img width="32" height="32" class="rounded" data-bs-toggle="tooltip" alt="">');
                                img.attr('src', icon).attr('data-bs-title', name || 'Archetipo');
                ...
                bindPrivateLogs: function () {
                    ...
                    root.on('click.profile-dm', '[data-action="profile-send-dm"]', function (event) {
                        event.preventDefault();
                        if (typeof window.GameOpenMessageModal !== 'function') { return; }
                        var ds = self.dataset || {};
                        var name = ((ds.name || '') + ' ' + (ds.surname || '')).trim();
                        window.GameOpenMessageModal(parseInt(ds.id, 10) || 0, name);
                    });
                ...
                hasAdminBypass: function () {
                    if (typeof window.Storage !== 'function') { return false; }
                ...
                getStoredProfileMusicMuted: function () {
                    ...
                    try {
                        return window.localStorage.getItem(this.profile_music_storage_key) === '1';
                    } catch (error) {
                        return false;
                    }
                },
                setStoredProfileMusicMuted: function (muted) {
                    ...
                    try {
                        window.localStorage.setItem(this.profile_music_storage_key, muted ? '1' : '0');
                    } catch (error) {}
                },
                ...
                openEditTabIfRequested: function () {
                    ...
                    if (window.location.pathname === '/game/profile') {
                        window.location.href = '/game/profile/edit';
                        return;
                    }
                },
                ...
                collectForm: function () {
                    ...
                    form.find(':input').each(function () {
                        ...
                        } else if (input.hasClass('summernote') && typeof $.fn.summernote === 'function') {
                ...
                buildEvents: function () {
                    ...
                    for (var i in this.events) {
                        ...
                        template.find('[name="body"]').text(row.body || '');
    ...
    window.GameProfilePage = GameProfilePage;
})(window);
