(function (window) {
            ...
            let page = {
                        ...
                        $(submitBtn).off('click.reportModal').on('click.reportModal', function () {
                            ...
                            var done = function () {
                                ...
                                if (window.Toast && typeof window.Toast.show === 'function') {
                                    window.Toast.show('Segnalazione inviata', 'success');
                                }
                            };
                            var fail = function (err) {
                                $(self2).prop('disabled', false);
                                var msg = window.Request && typeof window.Request.getErrorMessage === 'function'
                                    ? window.Request.getErrorMessage(err) : 'Errore nell\'invio della segnalazione.';
                                alert(msg);
                            };
                ...
                useAbilityQuickAction: function (abilityId, abilityName) {
                    ...
                    var started = this.callAbilities('use', payload, function (response) {
                        ...
                        if (window.GameFeatureError && typeof window.GameFeatureError.toastMapped === 'function') {
                            window.GameFeatureError.toastMapped(error, 'Uso skill non riuscito.', {
                                defaultType: 'error',
                ...
                useItemQuickAction: function (inventoryItemId, itemName) {
                    ...
                    var started = this.callInventory('useItem', payload, function () {
                        ...
                        if (typeof window.LocationSidebar !== 'undefined' && window.LocationSidebar && typeof window.LocationSidebar.reloadInventory === 'function') {
                            window.LocationSidebar.reloadInventory();
                        }
                        if (window.Toast && typeof window.Toast.show === 'function') {
                            window.Toast.show({
                ...
                send: function () {
                    ...
                    if (response && response.channel === 'whisper') {
                        if (typeof window.LocationWhispers !== 'undefined') {
                            window.LocationWhispers.appendMessage(response.dataset);
                        }
                        return;
                    }
    ...
    window.GameLocationChatPage = GameLocationChatPage;
})(window);
