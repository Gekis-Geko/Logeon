...
function GameMessagesPage(extension) {
        if (!globalWindow.__messagesInstances) {
            globalWindow.__messagesInstances = {};
        }

        let key = (extension && extension.key) ? extension.key : 'modal';
        if (globalWindow.__messagesInstances[key]) {
            if (extension) {
                Object.assign(globalWindow.__messagesInstances[key], extension);
            }
            return globalWindow.__messagesInstances[key];
        }
        ...
        let page = {
                ...
                if (recipientInput.length) {
                    recipientInput.off('input').on('input', function () {
                        ...
                        if (self.recipient_timer) {
                            globalWindow.clearTimeout(self.recipient_timer);
                        }
                        self.recipient_timer = globalWindow.setTimeout(function () {
            ...
            updateUnreadBadge: function (forcedCount) {
                ...
                if (count > 0 && globalWindow.AppSounds && typeof globalWindow.AppSounds.play === 'function') {
                    globalWindow.AppSounds.play('dm');
                }
        ...
        obj.key = key;
        globalWindow.__messagesInstances[key] = obj;
        return obj.init();
}
