(function () {
    ...
    var AdminCharacters = {
        ...
        handleAction: function (trigger, event) {
            ...
            var editActions = ['admin-edit-save-identity','admin-edit-save-narrative','admin-edit-save-stats','admin-edit-save-economy','admin-edit-save-notes','admin-edit-save-archetypes'];
        ...
        openEditModal: function (characterId) {
            ...
            this.loadArchetypesForEdit(characterId);
        },
        ...
        loadSocialStatusesForEdit: function () {
            ...
            this.requestPost('/admin/social-status/list', {}, function (r) {
        ...
        loadArchetypesForEdit: function (characterId) {
            var self = this;
            var tabBtn = document.querySelector('[data-role="admin-edit-tab-archetypes"]');
            var listNode = document.querySelector('[data-role="admin-edit-archetypes-list"]');
            var hintNode = document.querySelector('[data-role="admin-edit-archetypes-hint"]');
            ...
            this.archetypeConfig = null;

            this.requestPost('/admin/archetypes/config/get', {}, function (configResp) {
                ...
                self.archetypeConfig = config;
                if (tabBtn) { tabBtn.classList.remove('d-none'); }

                var multiple = parseInt(config.multiple_archetypes_allowed || 0, 10) === 1;
                if (hintNode) {
                    hintNode.textContent = multiple
                        ? 'Puoi assegnare più archetipi contemporaneamente.'
                ...
                function checkDone() {
                    ...
                    if (done === 2) {
                        self.currentArchetypeIds = (current || []).map(function (r) {
                            return parseInt(r.id, 10);
                        });
                        self.renderArchetypeList(config, available || [], self.currentArchetypeIds);
                    }
                }

                self.requestPost('/admin/archetypes/list', { is_active: 1, is_selectable: 1, results: 100 }, function (listResp) {
                    available = (listResp && Array.isArray(listResp.dataset)) ? listResp.dataset : [];
                    checkDone();
                });

                self.requestPost('/admin/archetypes/character/list', { character_id: characterId }, function (charResp) {
        ...
        renderArchetypeList: function (config, available, currentIds) {
            var listNode = document.querySelector('[data-role="admin-edit-archetypes-list"]');
            if (!listNode) { return; }
            ...
            if (available.length === 0) {
                listNode.innerHTML = '<div class="col-12"><p class="text-muted">Nessun archetipo selezionabile disponibile.</p></div>';
                return;
            }

            var html = '';
            for (var i = 0; i < available.length; i++) {
                var arch = available[i];
                var id = parseInt(arch.id, 10);
                ...
                html += '<div class="col-md-4 col-sm-6">'
                    ...
                    + '<label class="form-check-label" for="' + inputId + '">' + this.escapeHtml(arch.name || '') + '</label>'
                    + '</div>'
                    + '</div>'
                    + '</div>';
            }
        ...
        switchEditTab: function (tab, trigger) {
            var tabs = ['identity','narrative','stats','economy','notes','archetypes'];
            var modal = document.getElementById('admin-characters-edit-modal');
            if (!modal) { return; }
            ...
            if (trigger) { trigger.classList.add('active'); }
                ...
                var activeBtn = modal.querySelector('[data-action="admin-edit-tab"][data-tab="' + tab + '"]');
                if (activeBtn) { activeBtn.classList.add('active'); }
            }

            for (var i = 0; i < tabs.length; i++) {
                var panel = document.getElementById('admin-edit-panel-' + tabs[i]);
                if (panel) { panel.style.display = tabs[i] === tab ? '' : 'none'; }
            }
        },
        ...
        handleEditSave: function (section) {
            ...
            if (section === 'identity') {
                ...
                var selected = [];
                for (var j = 0; j < archetypeInputs.length; j++) {
                    var val = parseInt(archetypeInputs[j].value, 10);
                    if (val > 0 && selected.indexOf(val) === -1) { selected.push(val); }
                }

                var prevIds = self.currentArchetypeIds.slice();
                var toAssign = [];
                var toRemove = [];
                for (var ia = 0; ia < selected.length; ia++) {
                    if (prevIds.indexOf(selected[ia]) === -1) { toAssign.push(selected[ia]); }
                }
                for (var ir = 0; ir < prevIds.length; ir++) {
                    if (selected.indexOf(prevIds[ir]) === -1) { toRemove.push(prevIds[ir]); }
                }

                if (toAssign.length === 0 && toRemove.length === 0) {
                    Toast.show({ body: 'Nessuna modifica.', type: 'info' });
                    return;
                }

                var opsTotal = toAssign.length + toRemove.length;
                var opsDone = 0;
                function onOpDone() {
                    opsDone += 1;
                    if (opsDone === opsTotal) {
                        self.currentArchetypeIds = selected.slice();
                        Toast.show({ body: 'Archetipi aggiornati.', type: 'success' });
                    }
                }

                for (var assignIdx = 0; assignIdx < toAssign.length; assignIdx++) {
                    self.requestPost('/admin/archetypes/character/assign', { character_id: charId, archetype_id: toAssign[assignIdx] }, onOpDone);
                }
                for (var removeIdx = 0; removeIdx < toRemove.length; removeIdx++) {
                    self.requestPost('/admin/archetypes/character/remove', { character_id: charId, archetype_id: toRemove[removeIdx] }, onOpDone);
                }
                return;
            }
        ...
        requestPost: function (url, payload, onSuccess) {
    ...
    window.AdminCharacters = AdminCharacters;
})();
