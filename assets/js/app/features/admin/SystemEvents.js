...
var AdminSystemEvents = {
    ...
    initSwitches: function () {
        if (!this.eventForm || typeof globalWindow.SwitchGroup !== 'function') { return; }
        var input = this.eventForm.elements.show_on_homepage_feed;
        if (!input) { return; }
        this.homeFeedSwitch = globalWindow.SwitchGroup(input, {
            preset: 'yesNo',
            trueLabel: 'Visibile',
            falseLabel: 'Nascosto',
            trueValue: '1',
    ...
    searchScopeSuggestions: function () {
        ...
        var factionsListEndpoint = String(endpoints.factionsAdminList || '').trim();
        if (scopeType === 'faction' && !factionsListEndpoint) {
            this.hideSuggestions(box);
            return;
        }
        var endpoint = scopeType === 'faction' ? factionsListEndpoint : (scopeType === 'location' ? '/list/locations' : '/list/maps');
    ...
    searchParticipantSuggestions: function () {
        ...
        if (mode === 'faction') {
            var endpoints = globalWindow.LogeonModuleEndpoints || {};
            var factionsListEndpoint = String(endpoints.factionsAdminList || '').trim();
            if (!factionsListEndpoint) {
                this.hideSuggestions(box);
                return;
            }
            this.request(factionsListEndpoint, { limit: 400, page: 1 }, function (res) {
    ...
    searchCurrencySuggestions: function (input, hidden, box, role) {
        ...
        hidden.value = '0';
        var self = this;
        this.request(this.currencyListEndpoint(), { limit: 400, page: 1 }, function (res) {
            var rows = self.parseRows(res).map(function (r) {
    ...
    hideConfirm: function () {
        ...
        if (globalWindow.generalConfirm && typeof globalWindow.generalConfirm.hide === 'function') {
    ...
    debounce: function (key, delayMs, fn) {
        ...
        this.debounceTimers[id] = globalWindow.setTimeout(function () { fn(); }, Math.max(0, parseInt(delayMs, 10) || 0));
    },
...
globalWindow.AdminSystemEvents = AdminSystemEvents;
export { AdminSystemEvents as AdminSystemEvents };
