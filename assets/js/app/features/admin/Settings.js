(function () {
    ...
    var AdminSettings = {
        ...
        initSwitches: function () {
            ...
            this.switches.maps_view_mode = sw(jq('#s-maps-view-mode'), {
                trueLabel: 'Visuale',
                falseLabel: 'Card',
                trueValue: 'visual',
                falseValue: 'cards',
            ...
            if (soundsCard) {
                soundsCard.addEventListener('click', function (e) {
                    ...
                    if (testBtn) {
                        ...
                        if (window.AppSounds && typeof window.AppSounds.previewUrl === 'function') {
                            window.AppSounds.previewUrl(url);
                        }
                        return;
                    }
        ...
        save: function () {
            ...
            this.post('/admin/settings/update', payload, function (res) {
                ...
                self.showAlert('danger', 'Errore durante il salvataggio: ' + self.err(err));
                if (saveBtn) { saveBtn.disabled = false; }
            });
        },
        ...
        populateForm: function (d) {
            ...
            var switchKeys = ['onlines_auto_toast', 'presence_resume_last_position_on_signin', 'maps_view_mode', 'auth_google_enabled', 'multi_character_enabled', 'narrative_delegation_enabled'];
        ...
        collectPayload: function () {
            ...
            if (els['maps_view_mode']) {
                payload['maps_view_mode'] = els['maps_view_mode'].value;
            }

            var stringFields = ['auth_google_client_id', 'auth_google_client_secret', 'auth_google_redirect_uri'];
        ...
        buildSounds: function () {
            if (!window.AppSounds || typeof window.AppSounds.get !== 'function') {
                return;
            }
            var soundsCard = document.getElementById('admin-settings-sounds-card');
            ...
            for (var i = 0; i < types.length; i++) {
                var url = window.AppSounds.get(types[i]);
        ...
        saveSounds: function () {
            if (!window.AppSounds || typeof window.AppSounds.set !== 'function') {
        ...
        buildSoundUploader: function (uploadTarget, options) {
            ...
            var uploader = window.Uploader(config);
        ...
        err: function (error) {
            ...
            if (typeof window.Request !== 'undefined' && window.Request) {
                if (typeof window.Request.getErrorMessage === 'function') {
                    message = String(window.Request.getErrorMessage(error, '') || '').trim() || message;
                }
            }
