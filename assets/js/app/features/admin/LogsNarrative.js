...
var AdminLogsNarrative = {
    ...
    init: function () {
        ...
        this.root = document.querySelector('#admin-page [data-admin-page="logs-narrative"]');
        if (!this.root) {
            return this;
        }

        this.filtersForm = document.getElementById('admin-logs-narrative-filters');
        var gridEl = document.getElementById('grid-admin-logs-narrative');
    ...
    bind: function () {
        ...
        this.root.addEventListener('click', function (event) {
            ...
            if (action === 'admin-logs-narrative-reload') {
                event.preventDefault();
                self.loadGrid();
            }
        });
    ...
    initGrid: function () {
        ...
        this.grid = new Datagrid('grid-admin-logs-narrative', {
            name: 'AdminLogsNarrative',
            autoindex: 'id',
            orderable: true,
            thead: true,
            handler: { url: '/admin/logs/narrative/list', action: 'list' },
            ...
            columns: [
                ...
                },
                {
                    ...
                    sortable: false,
                    format: function (row) {
                        return self.renderData(row.data);
                    }
                },
                {
                    label: 'Data/Ora',
    ...
    buildFiltersPayload: function () {
        ...
        if (this.filtersForm) {
            ...
            var dateFrom = this.filtersForm.elements.date_from;
            if (dateFrom && dateFrom.value) {
                q.date_from = String(dateFrom.value).trim();
            }
            var dateTo = this.filtersForm.elements.date_to;
            if (dateTo && dateTo.value) {
                q.date_to = String(dateTo.value).trim();
            }
        }
    ...
    renderData: function (value) {
        ...
        try {
            ...
            return '<code class="small">' + this.escapeHtml(String(value)) + '</code>';
        }
        ...
        for (var i = 0; i < keys.length; i++) {
            var k = keys[i];
            parts.push(
                '<span class="text-muted small">' + this.escapeHtml(k) + ':</span> '
                + '<span class="small">' + this.escapeHtml(String(parsed[k])) + '</span>'
            );
        }
...
globalWindow.AdminLogsNarrative = AdminLogsNarrative;
export { AdminLogsNarrative as AdminLogsNarrative };
export default AdminLogsNarrative;
