...
var authzUiOff = null;
var EXTENSION_CONTROLLER_GLOBALS = [];
...
function normalizePageKeyToken(value) {
    return String(value || '').trim().toLowerCase().replace(/_+/g, '-').replace(/-page$/i, '');
}

function rememberExtensionGlobal(name) {
    var key = String(name || '').trim();
    if (!key) {
        return;
    }
    if (EXTENSION_CONTROLLER_GLOBALS.indexOf(key) === -1) {
        EXTENSION_CONTROLLER_GLOBALS.push(key);
    }
}

function normalizeControllerExtension(config) {
    if (!config || typeof config !== 'object') {
        return null;
    }

    var globalName = String(config.global || '').trim();
    if (!globalName) {
        return null;
    }

    var moduleName = String(config.module || '').trim();
    var moduleMethodName = String(config.moduleMethod || '').trim();
    var factoryName = String(config.factory || '').trim();
    ...
    if (moduleMethodName) {
        normalized.moduleMethod = moduleMethodName;
    }
    if (factoryName) {
        normalized.factory = factoryName;
    }

    return normalized;
}

function upsertControllerExtension(list, extension) {
    if (!Array.isArray(list) || !extension) {
        return;
    }

    var index = -1;
    for (var i = 0; i < list.length; i++) {
        if (String(list[i].global || '').trim() === extension.global) {
            index = i;
            break;
        }
    }
...
function registerSharedController(config) {
    var extension = normalizeControllerExtension(config);
    if (!extension) {
        return false;
    }

    rememberExtensionGlobal(extension.global);
    upsertControllerExtension(SHARED_CONTROLLER_EXTENSIONS, extension);
    return true;
}

function registerPageController(pageKey, config) {
    var key = normalizePageKeyToken(pageKey);
    if (!key) {
        return false;
    }
    ...
    if (!Array.isArray(PAGE_CONTROLLER_EXTENSIONS[key])) {
        PAGE_CONTROLLER_EXTENSIONS[key] = [];
    }

    rememberExtensionGlobal(extension.global);
    upsertControllerExtension(PAGE_CONTROLLER_EXTENSIONS[key], extension);
    return true;
}

function createRegisteredController(extension) {
    if (!extension || typeof extension !== 'object') {
        return null;
    }

    var config = {
        args: Array.isArray(extension.args) ? extension.args : [{}]
    };
    if (extension.module) {
        config.module = extension.module;
    }
    if (extension.moduleMethod) {
        config.moduleMethod = extension.moduleMethod;
    }
...
function initRegisteredControllers(list) {
    var items = Array.isArray(list) ? list : [];
    for (var i = 0; i < items.length; i++) {
        var extension = items[i];
        if (!extension || typeof extension !== 'object') {
            continue;
        }

        var globalName = String(extension.global || '').trim();
        if (!globalName) {
            continue;
        }

        cleanupController(globalName);
        window[globalName] = createRegisteredController(extension) || window[globalName];
    }
}

function resolveControllerGlobals() {
    var merged = PAGE_CONTROLLER_GLOBALS.slice();
    for (var i = 0; i < EXTENSION_CONTROLLER_GLOBALS.length; i++) {
        var name = EXTENSION_CONTROLLER_GLOBALS[i];
...
function initSharedWidgets() {
    initRegisteredControllers(SHARED_CONTROLLER_EXTENSIONS);
}

function initPageController() {
    var key = getPageKey();
    cleanupControllers(resolveControllerGlobals());
    ...
    if (key === 'chat-archives') {
        window.ChatArchives = createController({
            factory: 'GameChatArchivesPage',
            args: [{}]
        }) || window.ChatArchives;
        return;
    }
...
window.GamePage.registerSharedController = registerSharedController;
window.GamePage.registerPageController = registerPageController;
