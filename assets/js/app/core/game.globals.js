...
function initSummernote(context) {
    if (window.TipTapEditor && typeof window.TipTapEditor.init === 'function') {
        window.TipTapEditor.init(context || document);
        return;
    }
    ...
    if (root && root.querySelectorAll) {
        nodes = root.querySelectorAll('.summernote, .richtext-editor');
    } else {
        nodes = document.querySelectorAll('.summernote, .richtext-editor');
    }

    window.$(nodes).each(function () {
        window.$(this).summernote({
            height: 250
        });
    });
}
