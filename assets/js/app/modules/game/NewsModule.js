...
function createNewsModule() {
    return {
        ...
        widget: function (extension) {
            if (typeof globalWindow.GameNewsPage === 'function') {
                try {
                    return globalWindow.GameNewsPage(extension || {});
                } catch (error) {}
            }
        ...
        resolveListEndpoint: function () {
            var endpoints = (globalWindow.LogeonModuleEndpoints && typeof globalWindow.LogeonModuleEndpoints === 'object')
                ? globalWindow.LogeonModuleEndpoints
                : {};
            var endpoint = String(endpoints.newsList || '').trim();
            return endpoint;
        },
        ...
        list: function (payload) {
            ...
            var endpoint = this.resolveListEndpoint();
            if (!endpoint) {
                return Promise.resolve({ dataset: [] });
            }
            return this.request(endpoint, 'getNews', query);
        },
...
globalWindow.GameNewsModuleFactory = createNewsModule;
export { createNewsModule as GameNewsModuleFactory };
