...
class CharacterAttributesEngineService extends CharacterAttributesBaseService
{
    ...
    public function recomputeCharacter(int $characterId): array
    {
        ...
        foreach ($order as $attributeId) {
            ...
            if (!empty($rule) && isset($rule->round_mode) && $rule->round_mode !== null && $rule->round_mode !== '') {
    ...
    private function resolveNonDerivedBaseValue(object $definition, ?object $row, ?string &$source = null): float
    {
        if ($row !== null && isset($row->base_value) && $row->base_value !== null) {
            $source = 'base';
            return (float) $row->base_value;
        }

        if (isset($definition->default_value) && $definition->default_value !== null) {
            $source = 'default';
            return (float) $definition->default_value;
        }

        if (isset($definition->fallback_value) && $definition->fallback_value !== null) {
            $source = 'fallback';
        ...
        foreach ($steps as $step) {
            ...
            if ($operandType === 'attribute') {
                ...
                if (array_key_exists($operandAttributeId, $computed)) {
                    ...
                    if ($row !== null && isset($row->effective_value) && $row->effective_value !== null) {
                        $operand = (float) $row->effective_value;
                    } else {
                        $definition = $definitionById[$operandAttributeId] ?? null;
                        if ($definition !== null && isset($definition->default_value) && $definition->default_value !== null) {
                            $operand = (float) $definition->default_value;
                        } elseif ($definition !== null && isset($definition->fallback_value) && $definition->fallback_value !== null) {
