...
class ArchetypeService
{
    ...
    /**
     * Returns active and selectable archetypes together with the current config.
     */
    public function publicList(): array
    {
        $config = $this->getConfig();
        if ((int) ($config['archetypes_enabled'] ?? 1) !== 1) {
            return [
                'config' => $config,
                'dataset' => [],
            ];
    ...
    public function adminList(array $filters = [], int $limit = 20, int $page = 1, string $sort = 'sort_order|ASC'): array
    {
        ...
        $sortField = in_array($sortParts[0] ?? '', ['id', 'name', 'slug', 'sort_order', 'is_active', 'is_selectable', 'created_at'], true)
            ? ($sortParts[0] ?? 'sort_order') : 'sort_order';
